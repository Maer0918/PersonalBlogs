...
// 插入书籍表数据
books.forEach(book => {
	db.get("SELECT id FROM classes WHERE name = ?", book.className, (err, data) => {
...
/* 要使用 Promise 有机地将异步函数串行执行 */

// 插入文章表数据
articles.forEach(article => {
	db.get("SELECT id FROM books WHERE name = ?", article.bookName, (err, data) => {
		console.log(`article.bookName: ${article.bookName} book data: ${data}`)
...
headers.forEach(header => {
	db.get("SELECT id FROM articles WHERE name = ?", header.articleName, (err, data) => {
